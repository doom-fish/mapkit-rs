# mapkit coverage audit v2 (vs MacOSX26.2.sdk)
...
This is a v2 verification conducted against MacOSX26.2.sdk. The audit maintains the same symbol counts as v1, with both EXEMPT entries re-verified against the current SDK headers to confirm their deprecation status. This audit counts named top-level macOS declarations from MapKit.framework headers (interfaces, protocols, typedefs/enums/structs, exported constants, and top-level functions). Objective-C categories were not counted as standalone symbols.
