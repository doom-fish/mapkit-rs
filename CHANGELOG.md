...
- Added in-`src/` unit tests across `geometry.rs`, `error.rs`, and `selection_accessory.rs` (Tier 2 quality polish), providing fast `cargo test --lib` fail-fast signal alongside the existing integration tests under `tests/`.
