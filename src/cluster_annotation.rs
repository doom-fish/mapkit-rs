...
impl MKClusterAnnotation {
    pub fn new(member_annotations: &[MKPointAnnotation]) -> Result<Self, MapKitError> {
        let raw_members: Vec<*mut c_void> = member_annotations.iter().map(MKPointAnnotation::as_raw).collect();
        let mut error = ptr::null_mut();
        let raw = unsafe {
            ffi::mk_cluster_annotation_new(
                raw_members.as_ptr(),
                raw_members.len(),
                &mut error,
            )
    ...
    fn state(&self) -> Result<MKClusterAnnotationState, MapKitError> {
        let mut error = ptr::null_mut();
        let payload = unsafe { ffi::mk_cluster_annotation_state_json(self.raw.as_ptr(), &mut error) };
        if payload.is_null() {
            Err(unsafe {
    ...
    fn apply(&self, options: &MKClusterAnnotationOptions) -> Result<(), MapKitError> {
        ...
        let mut error = ptr::null_mut();
        unsafe { ffi::mk_cluster_annotation_apply_json(self.raw.as_ptr(), options.as_ptr(), &mut error) };
